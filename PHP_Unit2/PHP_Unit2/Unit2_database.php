...
function getNumberOfOrders($conn) {
    ...
    if($row['count'] == 0) {
        ...
        echo "Number of orders: "; echo $row['count'];
    }
}

function addOrder($product_id, $customer_id, $quantity, $price, $tax, $donation, $conn) {
...
function sellProduct($product_name, $quantity, $conn) {
    $sql = "UPDATE product 
            SET in_stock = CASE 
            ...
            WHERE product_name=?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iis", $quantity, $quantity, $product_name);
    $stmt->execute();
}
